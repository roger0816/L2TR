...
void LayerTr::slotReply(int iType,QByteArray data,QString sError)
{
        ...
        if(sMessage.trimmed().length()>0)
        {
            ...
            if(ui->rbAuto->isChecked())
            {
                if(!m_bLock)
                {
                    ...
                    QTimer::singleShot(2000,this,[=](){decodeFile(m_sPathCheck);});
                }
            }
...
void LayerTr::slotReplyAda(QByteArray data)
{
...
void LayerTr::doTr(int iIdx)
{
    ...
    if(file.open(QIODevice::ReadOnly))
    {
        ...
        if(m_iIdx==0)
            updateTx();
        m_api.sendChat("html翻譯成繁體中文 : "+read);
...
void LayerTr::decodeFile(QString sDirPath)
{

    QString sFilePath = m_listReadFile.at(m_iIdx);

    QString sFileNew = sFilePath.replace(m_sPathUnDone,sDirPath);

    saveFile(sFileNew,m_sOutData);


    QString sDelFile = m_listReadFile.at(m_iIdx);
...
void LayerTr::writeConf(QVariantMap data)
{
